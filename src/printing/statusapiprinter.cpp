                                         ...
                                         PrinterStatus *status, QString *err)
{
    ...
    // nType 2 = printer-queue name. The API takes an ANSI string; an empty name
    // is not accepted, so callers must resolve the default printer beforehand.
    QByteArray name = queueName.trimmed().toLocal8Bit();
    if (name.isEmpty()) {
        FreeLibrary(dll);
        return fail(QStringLiteral("No hay impresora configurada para la Status API."));
    }
    ...
    if (handle <= 0) {
        ...
        return fail(QStringLiteral("No se pudo abrir la impresora '%1' con la Status API (%2).")
                        .arg(queueName, QString::number(handle)));
    }
    ...
    if (!sent)
        return fail(QStringLiteral("Fallo al enviar los datos a '%1' con la Status API (%2).")
                        .arg(queueName, QString::number(rc)));
    return true;
#else
    Q_UNUSED(escpos);
    Q_UNUSED(queueName);
