...
| `UpdaterDialog` | `updaterdialog.h/.cpp` | Modal dialog wired to an `Updater`. Shows current vs. latest version + the GitHub release body as plain text in a `QTextEdit`. On "Actualizar ahora": collapses to a progress bar, drives `downloadInstaller`, then launches the installer + `qApp->quit()` so Inno Setup can replace the running files. On Windows it launches via `ShellExecuteW(…, "runas", …)` (raises the UAC prompt) rather than `QProcess::startDetached`: the installer targets Program Files and is manifested `requireAdministrator`, and `startDetached`/`CreateProcess` cannot elevate, so it would fail with `ERROR_ELEVATION_REQUIRED` unless the whole app was started as admin. Non-Windows keeps `startDetached`. |
