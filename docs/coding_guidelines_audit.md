...
| 9 | Many headers — `Imprimir`, `Contabilidad`, `Facturas`, `AddGarment`, `Listado`, `GenListado`, `InsertNewItem`, `CancelInvoiceDialog`, `RecogPrendas`, `MainWindow` | Public mutable members (e.g. `db`, `qrCode`, `isRecibo`, `revertirOn`, `pbAddedRows`, `isCellClicked`) instead of private + `m_` + accessor | Project pattern: parent assigns from outside (`dlg->db = db;`). Anti-encapsulation but pervasive. Refactoring to private accessors is a separate, larger task. |
