...
| `.github/scripts/Render-TestSummary.ps1` | Renders the foldable per-suite/per-method test report from `build/test-results-*.xml` into the GitHub step summary, and appends the `test_ticket_preview` ASCII receipt in a fenced block (GitHub strips inline images from summaries; the graphical SVG renders live in `docs/modules/printer/` + the `ticket-previews` artifact). Called by the `ci.yml` build job; also runnable locally. |
| `.github/workflows/ci.yml` | CI + Release in one workflow. **`build` job** (push/PR to `develop`/`master`/`feature/**`, and `X.Y` tags): Qt 6.4.3 MinGW + CMake + Ninja, `ctest`, uploads `laideal.exe` (on push) + `ticket-previews` PNG+SVG renders (always). **`release` job** (`needs: build`, only on `X.Y` tags): reuses the build exe -> `windeployqt` + zip + Inno Setup installer + publishes the GitHub Release. |
