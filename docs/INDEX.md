...
| `tests/` (`test_sql_lite`, `test_mysortfilterproxymodel`, `test_verifactu_response`, `test_backup_manager`, `test_contabilidad`) | Qt Test + CTest suites: sql_lite functions + `garmentImporte`, proxy diacritic filter, `parseVerifactuResponse`, `BackupManager::backupsToPrune` retention, `Contabilidad::periodRangeFor` date math. Run `ctest --test-dir build` |
