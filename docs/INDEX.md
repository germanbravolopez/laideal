...
| `tests/` (10 Qt Test + CTest suites) | `test_sql_lite` (+`garmentImporte`), `test_mysortfilterproxymodel`, `test_verifactu_response`, `test_verifactu_models`, `test_appsettings` (DPAPI), `test_facturas` (IVA split), `test_backup_manager`, `test_contabilidad`, `test_reporthtml`, `test_versioncompare`. Run `ctest --test-dir build` |
