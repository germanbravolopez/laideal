...
| `tests/` (`test_sql_lite`, `test_mysortfilterproxymodel`, `test_verifactu_response`, `test_verifactu_models`, `test_backup_manager`, `test_contabilidad`, `test_reporthtml`, `test_versioncompare`) | Qt Test + CTest suites: sql_lite + `garmentImporte`, proxy diacritic filter, `parseVerifactuResponse`, Verifactu config/invoice models, `BackupManager::backupsToPrune`, `Contabilidad::periodRangeFor`, `ReportHtml::formatEuro`, `Updater::compareVersions`. Run `ctest --test-dir build` |
