...
|----------|---------|---------|
| `migrateDatabase(db)` | `void` | Adds the `verifactu_*` columns to `ingresos` via `ALTER TABLE ADD COLUMN`, then runs the `SIN COBRAR` backfill (unpaid rows stamped `'PENDIENTE'` by the pre-10.9 `saveTicket`). Idempotent — skips existing columns and already-relabelled rows. Scoped to the literal `'PENDIENTE'`: NULL/`''` split-off rows are left alone because print/cancel queries detect them via `verifactu_estado != ''` |
...
| `pendingVerifactuEvents(db, floorIso)` | `QVector<PendingVerifactuEvent>` | Startup-recovery feed: one `{n_recibo, seq, fechaPago, cliente, importe}` per `(n_recibo, verifactu_invoice_seq)` still PENDIENTE/empty **and paid** (`pagado = 'SI'` with a non-empty `fecha_pago`) whose `fecha_recepcion` is on or after `floorIso`, each with its own `SUM(importe)`. Since the `SIN COBRAR` split an unpaid row carries its own estado and the estado filter already drops it; the payment gate is kept as belt-and-braces because it also covers legacy NULL/`''` rows, which the backfill leaves untouched (issue #43). Grouping by seq (no seq=0 filter) surfaces partial-pay events for recovery. `fechaPago` is the original AEAT submission date a retry must reuse (date is part of the invoice identity); the floor gates on `fecha_recepcion`. Ordered `n_recibo DESC, seq`. Drives `PendingSubmitsDialog` |
...
| `readLockForMonthAndYear(db, table, month, year)` | `int` | `1` if **any** of the month's rows are accounting-locked, `0` if open **or the month has no rows** (a valid empty SELECT returns 0 here — it does not distinguish "no data"); `2` only on a query error. Uses `COALESCE(MAX(edit_lock), 0)` so the lock state is order-independent — a voided garment (`edit_lock 0`, carrying a cancellation-stamped `fecha_pago`) can never mask a locked sibling and make a locked month read as open. Use `readLockForQuarter` when "no data" must be distinguished |
...
| `verifactuEventFor(db, nRecibo, seq)` | `PendingVerifactuEvent` | One payment event's own submission data, over its **paid** rows only: `SUM(importe)` and the original `fecha_pago`. Empty `nRecibo` when no paid event exists for that seq. Backs `RecogPrendas::retryVerifactuSubmit`, which must re-submit that event's amount under `verifactuInvoiceId(n, seq)` on the date AEAT keyed it on — not the whole ticket under the bare `n_recibo` on the reception date |
| `updateTicketVerifactuFields(db, nRecibo, result, seq)` | `void` | Writes an AEAT reply back onto a payment event's rows (csv / timestamp / estado / error / url_qr / xml / hash / invoice_id). Estado comes from `verifactuEstadoForResult()`, so a timeout / transport failure records **PENDIENTE and keeps its InvoiceID** (unknown outcome — AEAT may hold it) while only a definitive AEAT rejection becomes ERROR and clears the id. Scoped `WHERE n_recibo AND verifactu_invoice_seq AND pagado = 'SI'`. **The `pagado` predicate is load-bearing**: a ticket's *first* partial payment gets seq 0 (`nextVerifactuInvoiceSeq` counts paid rows, of which there are none yet) and the still-unpaid siblings are also seq 0, so scoping by seq alone stamped them ENVIADA + CSV for an invoice that never covered them. Shared by save-time submit, PayDialog, and both retry paths — all of which submit only paid rows |
