...
| `insertGarmentRow(db, IngresoGarmentRow)` | `bool` | RecogPrendas seam: insert the split-off garment row (SEPARATE_GARM 2/2). Leaves `verifactu_*` at empty defaults so the split row reads as legacy/NotSubmitted (re-submitting it would duplicate the ticket's AEAT InvoiceID) |
