...
**[COVERED]**
- QR is generated by Irene Solutions from the response (`result.qrCode`) and rastered into the ESC/POS receipt by `Imprimir::buildTicket()` (`GS v 0`, scaled to ~192 dots — the exact AEAT pixmap, not re-encoded). URL points to the correct AEAT endpoint per environment (`TESTING` vs `PRODUCTION`, see `VerifactuConfig::getValidationUrl()`). ✓
- Verification leyenda `Factura verificable en la sede electrónica de la AEAT` is printed immediately below the QR by `TicketRenderer` (centred, Font B). Guarded by `verifactu_estado == "ENVIADA"` (via `verifactuEstadoToString(VerifactuEstado::Enviada)`) so the claim never appears on PENDIENTE / ERROR / ANULADA rows. ✓
