...
| `PendingSubmitsDialog` (startup recovery) | Lists tickets left `PENDIENTE`/NULL/empty (per `n_recibo`, gated by `verifactu.pending_recovery_enabled` + floor date) and offers Reintentar / Error / Posponer. Scoped to `verifactu_invoice_seq = 0` (save-time / full-ticket submissions) — it re-submits a whole ticket as a bare `n_recibo`, so partial-pay events (`seq>0`) are intentionally excluded to avoid resubmitting the wrong amount/InvoiceID. |
