...
| Factura rectificativa UI | `src/app/` (new dialog), `src/verifactu/verifactuintegration.h/cpp` | Verifactu R1–R5 corrective invoice types are not exposed in the UI; today the only way to fix a wrong invoice is to cancel and resubmit, which doesn't cover partial corrections (price change, line correction). Add a separate dialog (similar to `CancelInvoiceDialog`) that searches an existing invoice by `n_recibo`, lets the user pick the rectification type (R1 most common — legal error), enters the corrected amount or differences, and submits via `submitInvoiceAsync()` with `InvoiceType = R1`/etc. + `RectificationType` + `RectificationTaxBase`/`RectificationTaxAmount`. The tax model already supports it (`VerifactuInvoice::RectificationItem` exists); the integration façade needs a new `submitRectificationAsync()` method. |
