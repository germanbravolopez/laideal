...
- [ ] **Verifactu — mandatory verification text next to the QR on printed receipts** (Req. 9; Disposición Final Primera RD 1007/2023) — the QR is present (`Imprimir::createTicketExcel()`, 140×140 at the bottom) but lacks the legally required text. Fix: in `imprimir.cpp`, immediately after `excel.insertImage(row, 1, qrImg)`, write a centred line `Factura verificable en la sede electrónica de la AEAT` (or the shorter `VERI*FACTU`) — only when the row has `verifactu_estado = 'ENVIADA'` (don't print the verification claim on tickets that were not actually submitted). The QR is also missing for save-time prints by design (DB still PENDIENTE) — that's correct; the text guard handles it.

- [ ] **Verifactu — XML export of `ingresos` records for Hacienda** (Req. 7; Art. 14.1 RD 1007/2023, Orden HAC/1177/2024) — the regulation requires "acceso completo e inmediato" in legible XML format (AEAT schema, UTF-8). Today there is no export action. Fix: the Irene Solutions response already returns the AEAT-style XML in `Return.Xml` of every `/Create` reply (visible in the logs). Persist it in a new `verifactu_xml` column on `ingresos` (TEXT) and add an action **Herramientas → Exportar registros AEAT (XML)** that concatenates the stored XMLs for a date range into a single envelope file at the user-chosen path. Requires `migrateDatabase()` to add the column and `VerifactuManager::processResponse()` to extract `Return.Xml` into `VerifactuResult::rawXml`.

- [ ] **Verifactu — Factura rectificativa UI (R1–R5)** (Req. 1; Art. 8.2.a RD 1007/2023 — corrections must use anulación *or* rectificativa) — promoted from Non-Blocking. Today the only way to fix a wrong invoice is to cancel and resubmit, which does not cover partial corrections (price changes, line corrections). Fix: new dialog modelled on `CancelInvoiceDialog`; user searches by `n_recibo`, picks rectification type (R1 most common — legal error), enters the corrected amount or differences, and submits via a new `VerifactuIntegration::submitRectificationAsync()` that builds a `VerifactuInvoice` with `InvoiceType = R1`/etc. + `RectificationType` (S/I) + `RectificationTaxBase` / `RectificationTaxAmount`. The tax model already supports it (`VerifactuInvoice::RectificationItem` exists in the dead-code report — make it live).

- [ ] **Verifactu — persist chained hash (`Huella`) from AEAT response** (Req. 3; Art. 12 RD 1007/2023) — AEAT/IreneSolutions computes the SHA-256 chained hash and returns it as `<sum1:Huella>...</sum1:Huella>` in the SOAP XML of every `/Create` reply. Today we store the CSV but discard the Huella. Fix: add a `verifactu_huella` column to `ingresos` (CHAR(64) hex), extract `<Huella>` from the `Return.Xml` field in `VerifactuManager::processResponse()`, and persist it via the existing async update path. Strengthens local tamper-detection independently of the AEAT side.

- [ ] **Printed ticket columns too wide for printer** — explicit `setColumnWidth` calls (5+21+8=34 units total, added in `d9627d2`) exceeded the thermal printer's page width, splitting the Excel across multiple horizontal print areas. Column widths reduced to (4, 20, 7.5) = 31.5 units; `excel.setPageMargins(0.6, 0.6, 0.4, 0.4)` added to control the printable area (new QXlsx API — see Completed Milestone "QXlsx — page-margin support"). Excel output verified locally; **still needs testing on the physical thermal printer** before this entry can move to Completed.
...
| Bundle icon + Excel template with installer | `releases/laideal.iss`, `src/appsettings/appsettings.cpp`, `~/.laideal_settings.json`, `SettingsDialog` | Today the user must set `icon` and `print.template` paths in Configuración after install — these are app-internal resources the user shouldn't touch (or accidentally delete). Ship both files inside the installer (`Program Files/Laideal/resources/`) and have `AppSettings` resolve them via `QCoreApplication::applicationDirPath()` instead of free-form paths. Remove the corresponding fields from `SettingsDialog`. Migration: on app start, if the JSON still points to a user path, ignore it and use the bundled default. |
| Single user-configurable reports root path | `src/appsettings/`, `~/.laideal_settings.json`, `SettingsDialog`, `src/contabilidad/`, `src/listado/` | Today the user configures three separate output paths (`contabilidad.path`, `listados.prendas`, `listados.gastos`). Replace with one `reports.root` path; the app uses hardcoded subfolders (`contabilidad/`, `listados/prendas/`, `listados/gastos/`) created on demand via `QDir::mkpath`. Simpler UX, fewer chances for the user to misconfigure. Migration: on first run with the legacy three keys present, fold them into a sensible `reports.root` (e.g. their common ancestor) and warn once. |
